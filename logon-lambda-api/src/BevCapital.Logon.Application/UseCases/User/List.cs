...
namespace BevCapital.Logon.Application.UseCases.User
{
    public class List
    {
        public class ListAppUserQuery : IRequest<List<AppUserOut>> { }

        public class Handler : IRequestHandler<ListAppUserQuery, List<AppUserOut>>
        {
            ...
            public async Task<List<AppUserOut>> Handle(ListAppUserQuery request, CancellationToken cancellationToken)
            {
                var cachedAppUsers = await _distributedCache.GetAsync<List<AppUserOut>>(CacheKeys.LIST_ALL_USERS);
                if (cachedAppUsers == null || !cachedAppUsers.Any())
                {
                    var appUsers = await _unitOfWork.Users.GetAllAsync(cancellationToken);
                    cachedAppUsers = _mapper.Map<List<AppUserOut>>(appUsers);

                    await _distributedCache.SetAsync<List<AppUserOut>>
                                                    (
